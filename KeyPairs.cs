...
namespace InfoSec.RestApi
{
    ...
    public class KeyPairs : ControllerBase
    {
        ...
        public async Task<IActionResult> CheckSign([FromForm] CheckSignDto dto)
        {
            ...
            var alicaHash = BitConverter.ToInt16(tmpHashA);//хэш, который сгенирировала Алиса из полученного документа
        ...
        public async Task<IActionResult> Sign([FromForm] SignDto dto)
        {
            ...
            var file = dto.File.ToByteArray();
            byte[] tmpHashB;
            tmpHashB = new MD5CryptoServiceProvider().ComputeHash(file);
            
            // Todo: Сгенерировать подпись
 
            var hashFunction = BitConverter.ToInt16(tmpHashB);//хэш, который сгенирировал Боб
